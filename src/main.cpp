...
#if defined(NDEBUG)
# error "AmburCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8197f6f4092bd0d7d19fc48d7d35d7052cf3c5264daa84ef07930d1c505f4952");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AmburCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AmburCoin Signed Message:\n";
...
{
    // AmburCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 100000 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AmburCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AmburCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1522901429, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Trump Plans to Send National Guard";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044315e67d533c190c008a3d983c02eab5e9f08767a2064186bb14477bd8c3cc2cf4c7b3edce5f5d0e6bbe33885d8ea07a518262ede5d8e56bdd32ebef28d71a77") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522901429;
        ...
        assert(block.hashMerkleRoot == uint256("0x5a74a374ab94a47c32b91de26f1b9b9d38c43620dc8a3492aa6626553b325c7d"));
...
unsigned char pchMessageStart[4] = { 0xa6, 0x1f, 0x65, 0x5d }; // AmburCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AmburCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AmburCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AmburCoinMiner(CWallet *pwallet)
{
    printf("AmburCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("amburcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AmburCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AmburCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AmburCoinMiner, pwallet));
}
